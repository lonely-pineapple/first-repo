...
## 4.1. Create the logistic regression model
...
Our AUC is ***0.654*** when back-testing on 40% of the 
...
for(i in seq(from=-1, to=8, by=increment)) {             
...
ggplot(forecast_2, aes(y=values, x=change_in_price, fill=effect))+
  geom_line()+
  geom_segment(aes(x = min_price, y = max_revenue, xend = price_max_revenue, yend = max_revenue, colour="red"))+
  geom_segment(aes(x = price_max_revenue, y = min_profit, xend = price_max_revenue, yend = max_revenue, colour="red"))+
  geom_segment(aes(x = min_price, y = max_profit, xend = price_max_profit, yend = max_profit, colour="blue"))+
  geom_segment(aes(x = price_max_profit, y = min_profit, xend = price_max_profit, yend = max_profit, colour="blue"))+
  geom_segment(aes(x = min_price, y = next_year_revenue, xend = max_price, yend = next_year_revenue, colour="green"))+
  geom_segment(aes(x = min_price, y = next_y_profit, xend = max_price, yend = next_y_profit, colour="yellow"))+
  scale_color_hue(labels = c(max_profit_full, actual_rev_full, max_rev_full, actual_profit_full))+
  theme_bw()+
...
ggplot(forecast_3, aes(y=number_of_contracts_sold, x=change_in_price))+
  geom_line(size=1.25)+
  theme_bw()+
  geom_segment(aes(x = min_price, y = qty_max_revenue, xend = price_max_revenue, yend = qty_max_revenue, colour="red"), size = 1, linetype = 3)+
  geom_segment(aes(x = price_max_revenue, y = 0, xend = price_max_revenue, yend = qty_max_revenue, colour="red"), size = 1, linetype = 3)+
  geom_segment(aes(x = min_price, y = qty_max_profit, xend = price_max_profit, yend = qty_max_profit, colour="blue"), size = 1, linetype = 3)+
  geom_segment(aes(x = price_max_profit, y = 0, xend = price_max_profit, yend = qty_max_profit, colour="blue"), size = 1, linetype = 3)+
  geom_segment(aes(x = min_price, y = next_y_contracts_sold, xend = max_price, yend = next_y_contracts_sold, colour="green"), size = 1, linetype = 2)+
  scale_color_hue(labels = c(qty_prof_max_full, qty_act_full, qty_rev_max_full))+
